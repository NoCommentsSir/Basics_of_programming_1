...
struct List {
    ...
    void Sort(void (*sorting)(List &list)){
        sorting(*this);
    }
};
    void sortQ(List& list, int first, int last) {
        ...
        if (f < last) sortQ(list, f, last);
        if (first < l) sortQ(list, first, l);
    }
    void sortQ(List& list){
        sortQ(list, 0, list.count()-1);
    }
